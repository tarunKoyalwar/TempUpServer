...
class requesthandler(CGIHTTPRequestHandler):
    def do_GET(self):
        ...
        """ 
            ...
            self.send_header("content-type",'text/html')
            self.end_headers()
            self.wfile.write('Backdoor Access Denied'.encode()) 
    ...
    def deal_post_data(self):
        ...
        if fileitem.filename:
            ...
            try:
                ...
                with open(os.getcwd()+seperator+fn,'wb') as file:
                    file.write(fileitem.file.read())
                    print(f"File uploded {fn} by POST from {self.client_address}")
            return True
        else:
            return False
...
if __name__ == '__main__':
    parser=argparse.ArgumentParser(description="A Simple http server for upload and download for temporary use")
